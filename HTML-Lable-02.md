...
1.   语义化是指用合理的HTML标签以及器特有的属性去格式化文档内容，通俗的讲语义化就是对数据和信息进行处理，使得机器可以理解。
2.   一般的网站分为头部、导航、文章(或其他模块)、侧栏、底部、根据不同的部位，使用不同的标签进行书写。
     -    表示页面不同位置的标签有：header 、nav、article、section、footer、aside
     -    表示具体元素的作用或意义的标签：a、abbr、address、audio、blockquote、caption、code、detalist、del、detail、ol、ul、figure、figuration、img、input、mark、p......

#### 语义化的优点

1.   对搜素引擎友好,有利于SEO和网络爬虫的抓取.
2.   有利于HTML文件的裸奔,即去除CSS样式,网页仍然有良好的可读性.
3.   title 和 alt 等属性,对用户友好
4.   有利于代码的维护
5.   对于移动设备,盲人阅读器等身背友好,能够使其更好的解读HTML网页.
